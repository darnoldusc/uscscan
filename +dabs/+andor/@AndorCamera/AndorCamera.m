classdef AndorCamera < most.APIWrapper & most.PDEPProp
    ...
    properties (Constant, Hidden)
        ...
        apiSupportedVersionNames = {'2.91'}; %A cell array of shorthand names (strings) for API versions supported by this wrapper class
        
        apiDLLNames = zlclPlatformProps('apiDLLNames'); %version-indexed. The name of the DLL sans the '.dll' extension.
        apiHeaderFilenames = zlclPlatformProps('apiHeaderFilenames'); %version-indexed. The name of the header file (with the '.h' extension - OR a .m or .p extension).
    ...
    properties (SetAccess=protected, Hidden)
        ...
        apiHeaderRootPath = fullfile(fileparts(mfilename('fullpath'))),'private'); %NOT version-indexed. Either 'class', 'package', or <real path>. 'class' and 'package' indicate class or package private path, respectively.
...
%% LOCAL HELPERS
function val = zlclPlatformProps(propName)

assert(ispc());
is64 = strcmpi(computer(),'pcwin64');
...
switch propName
    ...
    case 'apiHeaderFilenames'
        if is64
            val = 'atmcd64d_proto.m';
        else
            val = 'atmcd32d_proto.m';
        end
