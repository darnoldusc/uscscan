classdef LSM < dabs.thorlabs.private.ThorDevice
    ...
    methods (Hidden, Access=protected)
        ...
        function pdepPropHandleSet(obj,src,evnt)
            ...
            disallowWhileRunning = {'triggerMode' 'multiFrameCount' 'channelsLogging' 'channelsViewing' 'circBufferSize' 'loggingAveragingFactor' 'frameEventDecimationFactor' 'dataMappingMode' 'captureWithoutScanner' 'areaMode' 'galvoChanEnable'};
            ...
            if strcmpi(obj.state,'active') && ~ismember(propName, alwaysAllow)
                ...
                elseif ismember(propName, obj.CONFIG_BUFFER_PARAMS) && obj.logging
            ...
            if obj.initialized
                activeNow = isequal(obj.state,'active');
                if  activeNow && obj.restartOnParamChange
                    obj.startAlreadyRunning();
                elseif ~activeNow && ismember(propName,obj.CONFIG_BUFFER_PARAMS)
                    obj.configureFrameAcquiredEvent('configBuffers');
                end
            end
    ...
    methods
        ...
        function startAlreadyRunning(obj,setParamFcn,varargin)
            ...
            %
            % setParamFcn: <Optional> Handle to function used to set parameter between stopping LSM acquisition and restarting it
            % varargin: Arguments to setParamFcn, as needed            
            ...
            if strcmpi(obj.state,'active')
                ...
                needsConfig = any(ismember(obj.paramChangeList,obj.CONFIG_BUFFER_PARAMS));
                
                if ~isempty(setParamFcn) || needsConfig                    
                    
                    obj.configureFrameAcquiredEvent('stop'); %This stops ThorFrameCopier processing; does NOT call postflightAcquisition()
                    
                    if nargin > 1 && ~isempty(setParamFcn)
                        feval(setParamFcn,varargin{:});
                    end

                    %Handle parameter changes which require call to configBuffers.
                    if any(ismember(obj.paramChangeList,obj.CONFIG_BUFFER_PARAMS))                                            
                        obj.configureFrameAcquiredEvent('configBuffers');
                    end
